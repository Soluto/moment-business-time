...
moment.fn.isEventDowntime = function isEventDowntime(start, end) {
    return this.isSameOrAfter(start) && this.isSameOrBefore(end);
}

moment.fn.isEventDowntimeByConfig = function isEventDowntimeByConfig(downtimeHours) {
    try {
        if (isEmptyObject(downtimeHours)) {
            return true;
        }
        ...
        const localMoment = moment(new Date(new Date().toLocaleString('en-US', {timeZone: downtimeHours.tz})));
        localMoment.locale(localeName);

        return localMoment.isEventDowntime(downtimeHours.start, downtimeHours.end);
    } catch (e) {
        throw Error('failed parsing event downtime hours. error:' + e)
    }
};
